...
SCHEMA = "0.9.0"
RUN_ID = "dark-academia-membership-recursion-wave-3-2026-07-31"
...
class Scraper:
    ...
    def run(self) -> None:
        ...
        existing_manifest = self.root / "db" / "dataset-manifest" / "starintel:dataset-manifest:dark-academia.ndjson"
        if existing_manifest.exists():
            try:
                prior = json.loads(existing_manifest.read_text(encoding="utf-8").splitlines()[0])
                previous = [x.get("component_dataset") for x in prior.get("data", {}).get("target_options", []) if x.get("component_dataset")]
                component_datasets = list(dict.fromkeys(previous + component_datasets))
            except Exception:
                pass
