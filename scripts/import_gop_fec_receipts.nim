...
proc gzip(inputPath, outputPath: string) =
  let process = startProcess(
    "sh",
    args = ["-c", "gzip -9 -n -c " & quoteShell(inputPath) & " > " & quoteShell(outputPath)],
    options = {poUsePath}
  )
...
proc encodeParts(gzipPath, outputDir: string): seq[string] =
  ...
  proc startPart() =
    ...
    current = open(outputDir / name, fmWrite)
    result.add(name)
    currentBytes = 0
    openPart = true
    inc partIndex
  ...
  while true:
    ...
    while offset < encoded.len:
      if not openPart:
        startPart()
      let take = min(PartSize - currentBytes, encoded.len - offset)
      current.write(encoded[offset ..< offset + take])
      currentBytes += take
      offset += take
      if currentBytes == PartSize:
        closePart()
...
proc obtainZip(options: Options; destination: string) =
  ...
  let process = startProcess(
    "curl",
    args = ["--fail", "--location", "--retry", "3", "--output", destination, bulkUrl(options.cycle)],
