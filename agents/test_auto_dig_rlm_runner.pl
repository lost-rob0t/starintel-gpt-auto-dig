...
test(native_direct_mode_features_are_explicitly_enabled) :-
    auto_dig_runtime_options('z-ai/glm-5.3-flash', none, Options),
...
test(glm_flash_context_budget_is_thirty_percent_of_model_limit) :-
    auto_dig_context_budget('z-ai/glm-5.3-flash', Window, Budget),
    assertion(Window =:= 1310720),
    assertion(Budget =:= 393216).

test(historical_gpt_context_budgets_remain_explicit) :-
...
test(research_prompt_requires_plain_terminal_report) :-
    auto_dig_query(Query),
    assertion(sub_string(Query, _, _, _, "terminal response MUST be plain evidence-backed report prose")),
    assertion(sub_string(Query, _, _, _, "not tool-call syntax")),
    assertion(sub_string(Query, _, _, _, "not")),
    assertion(sub_string(Query, _, _, _, "statement that the run merely completed")).
...
test(short_status_text_is_not_a_final_actor_answer) :-
    Result = _{response:_{assistant:_{content:"run complete"}}},
    assertion(\+ outcome_final_answer(ok(Result), _)).
