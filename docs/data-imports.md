...
Network collection requires explicit acknowledgement of the written authorization with `--authorized` or `INFLUENCEWATCH_AUTHORIZED=1`. The authorized client enforces a minimum one-second interval between requests and generates a unique user agent for every run. The complete user-agent value must match:

```regex
^[0-9a-f]{64}$
```

The value is generated by `openssl rand -hex 32`. Authorized runs do not enforce `robots.txt` unless `--respect-robots` is explicitly supplied. The importer retries transient failures and caps response sizes.
